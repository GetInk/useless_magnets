...
keydict = {"space": pygame.K_SPACE, "lshift": pygame.K_LSHIFT, "rshift": pygame.K_RSHIFT, "esc": pygame.K_ESCAPE, "up": pygame.K_UP, "down": pygame.K_DOWN,
...
keydictdrop = {"space": pygame.K_SPACE, "lshift": pygame.K_LSHIFT, "rshift": pygame.K_RSHIFT, "esc": pygame.K_ESCAPE, "up": pygame.K_UP, "down": pygame.K_DOWN,
           "left": pygame.K_LEFT, "right": pygame.K_RIGHT,
           "a": pygame.K_a,
           "b": pygame.K_b,
           "c": pygame.K_c,
...
class newSprite(pygame.sprite.Sprite):
    ...
    def changeImageNoSmoothing(self, index):
        orig_size = self.images[self.currentImage].get_rect()
        self.currentImage = index
        if self.angle == 0 and self.scale == 1:
            self.image = self.images[index]
        else:
            self.tempImage = pygame.transform.scale(self.images[self.currentImage], (orig_size[2]*self.scale, orig_size[3]*self.scale))
            self.tempImage = pygame.transform.rotate(self.tempImage, -self.angle)
        oldcenter = self.rect.center
        self.rect = self.image.get_rect()
        originalRect = self.images[self.currentImage].get_rect()
        self.originalWidth = originalRect.width
...
def screenSize(sizex, sizey, Screen_Name, xpos=None, ypos=None, fullscreen=False):
    global screen
    global background
...
def transformSpriteNoSmoothing(sprite, angle, scale, hflip=False, vflip=False):
    orig_size = sprite.images[sprite.currentImage].get_rect()
    oldmiddle = sprite.rect.center
    if hflip or vflip:
        tempImage = pygame.transform.flip(sprite.images[sprite.currentImage], hflip, vflip)
    else:
    ...
    if angle != 0 or scale != 1:
        sprite.angle = angle
        sprite.scale = scale
        tempImage = pygame.transform.scale(tempImage, (orig_size[2]*scale, orig_size[3]*scale))
        tempImage = pygame.transform.rotate(tempImage, -angle)
...
def setBackFgroundColour(colour):
...
def changeSpriteImageNoSmoothing(sprite, index):
    sprite.changeImageNoSmoothing(index)
